 ...
 */
private data class TimingResult<T>(val times: List<Duration>, val lastValue: T) {
...
class TrajectoryTimingTest {
  ...
  fun `measure individual step timing`() {
    ...
    val angularSamplingResult = measureWithWarmup {
      posePaths.map { posePath ->
        samplePathByRotation(
          posePath,
          TEST_PROFILE_PARAMS.angResolution,
          TEST_PROFILE_PARAMS.angSamplingEps,
        )
      }
    }
    ...
    println(
      "  Position Path Building: ${positionPathResult.average.toString(DurationUnit.MILLISECONDS, 3)}",
    )
    println(
      "  Pose Path Building:     ${posePathResult.average.toString(DurationUnit.MILLISECONDS, 3)}",
    )
  ...
  fun `measure full trajectory builder timing`() {
    ...
    println(
      "Total Average: ${(builderResult.average + buildResult.average).toString(
  ...
  fun `measure timing scaling with complexity`() {
    ...
    println(
      "(Warmup: $WARMUP_ITERATIONS iterations, Measured: $MEASURED_ITERATIONS iterations per complexity level)\n",
    )
  ...
  fun `demonstrate warmup effect`() {
    ...
    println(
      "Speedup: ${"%.2fx".format(
        coldResult.duration.toDouble(
          DurationUnit.MILLISECONDS,
        ) / warmAvg.toDouble(DurationUnit.MILLISECONDS),
      )}",
    )
    println()
  }
  ...
  fun `compare DisplacementProfile vs TimeProfile`() {
    ...
    println(
      "  Per-lookup avg: ${"%.3f".format(
        dispLookupResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println()
    ...
    println(
      "  Per-lookup avg: ${"%.3f".format(
        timeLookupResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println()
    ...
    println(
      "  Per-lookup avg: ${"%.3f".format(
        inverseResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println()
    ...
    println("Generation Time:")
    println(
      "  CancelableProfile (w/ DisplacementProfile): ${dispProfileResults.average.toString(
        DurationUnit.MILLISECONDS,
        3,
      )}",
    )
    println(
      "  TimeProfile (from DisplacementProfile):     ${timeProfileResult.average.toString(
        DurationUnit.MILLISECONDS,
        3,
      )}",
    )
    println(
      "  Combined (both):                            ${combinedResult.average.toString(
        DurationUnit.MILLISECONDS,
        3,
      )}",
    )
    ...
    println(
      "  DisplacementProfile[x]:  ${"%.3f".format(
        dispLookupResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println(
      "  TimeProfile[t]:          ${"%.3f".format(
        timeLookupResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println(
      "  TimeProfile.inverse(x):  ${"%.3f".format(
        inverseResult.average.toDouble(DurationUnit.MICROSECONDS) / numLookups,
      )} µs",
    )
    println()
