...
class RamseteTest {
  ...
  @Test
  fun testNewRamseteReversed() = testNewRamseteHelper(true)
...
class OldRamseteController(trackWidth: Double, val zeta: Double = 0.7, bBar: Double = 2.0) {
...
fun <Param : DualParameter> PoseVelocity2dDual<Param>.flatten(): List<Double> =
  linearVel.x.values() + linearVel.y.values() + angVel.values()
