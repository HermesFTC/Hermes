...
class RefDualNum<Param : DualParameter>(val values: List<Double>) {
  companion object {
    fun <Param : DualParameter> constant(x: Double, n: Int): RefDualNum<Param> = if (n <= 1) {
      RefDualNum(listOf(x))
    } else {
      RefDualNum(listOf(x) + List(n - 1) { 0.0 })
    }

    fun <Param : DualParameter> variable(x: Double, n: Int): RefDualNum<Param> = when {
...
internal fun <Param : DualParameter> assertDualEquals(
  expected: DualNum<Param>,
  actual: DualNum<Param>,
