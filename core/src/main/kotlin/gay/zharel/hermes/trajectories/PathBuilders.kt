...
class PositionPathSeqBuilder private constructor(
  ...
  fun endPath() = PositionPathSeqBuilder(
    eps,
    if (segments.isEmpty()) {
      paths
    } else {
  ...
  fun setTangent(newTangent: Rotation2d) = if (abs(nextBeginTangent - newTangent) < 1e-6) {
    this
  } else {
    val b = endPath()
   ...
   */
  fun forward(ds: Double): PositionPathSeqBuilder = addSegment(
    Line(
      nextBeginPos,
  ...
  fun lineToY(posY: Double): PositionPathSeqBuilder {
    ...
    return addSegment(
      Line(
        nextBeginPos,
        Vector2d(
          (posY - nextBeginPos.y) / nextBeginTangent.imag * nextBeginTangent.real + nextBeginPos.x,
          posY,
        ),
      ),
    )
...
private fun <Param : DualParameter> DualNum<Param>.withValue(x: Double) = DualNum<Param>(
  DoubleArray(size()) {
    if (it == 0) {
      x
    } else {
...
class PosePathSeqBuilder private constructor(
  ...
  private class Eager(val segments: List<PosePath>, val endHeadingDual: Rotation2dDual<Arclength>) :
    State {
  ...
  private fun viewUntil(disp: Double) = PositionPathView(posPath, endDisp, disp - endDisp)
...
class PathBuilder private constructor(
  ...
  fun setTangent(r: Rotation2d) = PathBuilder(
    beginHeading,
    positionPathSeqBuilder.setTangent(r),
    headingSegments,
    endHeading,
  )
  ...
  fun forwardLinearHeading(ds: Double, heading: Double) = forwardLinearHeading(ds, Rotation2d.exp(heading))
  ...
  fun forwardSplineHeading(ds: Double, heading: Double) = forwardSplineHeading(ds, Rotation2d.exp(heading))
  ...
  fun lineToXConstantHeading(posX: Double) = copy(
    positionPathSeqBuilder.lineToX(posX),
    headingSegments + listOf {
      constantUntil(it)
    },
    endHeading,
  )

  fun lineToXLinearHeading(posX: Double, heading: Rotation2d) = copy(
    positionPathSeqBuilder.lineToX(posX),
    headingSegments + listOf {
      linearUntil(it, heading)
    },
    heading,
  )
  fun lineToXLinearHeading(posX: Double, heading: Double) =
    lineToXLinearHeading(posX, Rotation2d.exp(heading))

  fun lineToXSplineHeading(posX: Double, heading: Rotation2d) = copy(
    positionPathSeqBuilder.lineToX(posX),
    headingSegments + listOf {
      splineUntil(it, heading)
    },
    heading,
  )
  fun lineToXSplineHeading(posX: Double, heading: Double) =
    lineToXSplineHeading(posX, Rotation2d.exp(heading))
  ...
  fun lineToYConstantHeading(posY: Double) = copy(
    positionPathSeqBuilder.lineToY(posY),
    headingSegments + listOf {
      constantUntil(it)
    },
  ...
  fun lineToYLinearHeading(posY: Double, heading: Rotation2d) = copy(
    positionPathSeqBuilder.lineToY(posY),
    headingSegments + listOf {
      linearUntil(it, heading)
    },
  ...
  fun lineToYLinearHeading(posY: Double, heading: Double) =
    lineToYLinearHeading(posY, Rotation2d.exp(heading))

  fun lineToYSplineHeading(posY: Double, heading: Rotation2d) = copy(
    positionPathSeqBuilder.lineToY(posY),
    headingSegments + listOf {
      splineUntil(it, heading)
    },
  ...
  fun lineToYSplineHeading(posY: Double, heading: Double) =
    lineToYSplineHeading(posY, Rotation2d.exp(heading))
  ...
  fun strafeToConstantHeading(pos: Vector2d) = copy(
    positionPathSeqBuilder.strafeTo(pos),
    headingSegments + listOf {
      constantUntil(it)
    },
  ...
  fun strafeToLinearHeading(pos: Vector2d, heading: Rotation2d) = copy(
    positionPathSeqBuilder.strafeTo(pos),
    headingSegments + listOf {
      linearUntil(it, heading)
    },
  ...
  fun strafeToLinearHeading(pos: Vector2d, heading: Double) =
    strafeToLinearHeading(pos, Rotation2d.exp(heading))

  fun strafeToSplineHeading(pos: Vector2d, heading: Rotation2d) = copy(
    positionPathSeqBuilder.strafeTo(pos),
    headingSegments + listOf {
      splineUntil(it, heading)
    },
  ...
  fun strafeToSplineHeading(pos: Vector2d, heading: Double) =
    strafeToSplineHeading(pos, Rotation2d.exp(heading))

  fun splineTo(pos: Vector2d, tangent: Rotation2d) = copyTangent(
    positionPathSeqBuilder.splineTo(pos, tangent),
    headingSegments + listOf { tangentUntil(it) },
  )
  fun splineTo(pos: Vector2d, tangent: Double) = splineTo(pos, Rotation2d.exp(tangent))

  fun splineToConstantHeading(pos: Vector2d, tangent: Rotation2d) = copy(
    positionPathSeqBuilder.splineTo(pos, tangent),
    headingSegments + listOf {
      constantUntil(it)
    },
  ...
  fun splineToLinearHeading(pose: Pose2d, tangent: Rotation2d) = copy(
    ...
    headingSegments + listOf { linearUntil(it, pose.heading) },
    pose.heading,
  )
  fun splineToLinearHeading(pose: Pose2d, tangent: Double) =
    splineToLinearHeading(pose, Rotation2d.exp(tangent))

  fun splineToSplineHeading(pose: Pose2d, tangent: Rotation2d) = copy(
    positionPathSeqBuilder.splineTo(pose.position, tangent),
    headingSegments + listOf { splineUntil(it, pose.heading) },
    pose.heading,
  )
  fun splineToSplineHeading(pose: Pose2d, tangent: Double) =
    splineToSplineHeading(pose, Rotation2d.exp(tangent))
