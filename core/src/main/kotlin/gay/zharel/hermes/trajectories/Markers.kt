...
class AfterDispTrigger(val disp: Double) : MarkerTrigger {
  override fun shouldTrigger(robotState: RobotState, trajectory: Trajectory<*>, s: Double): Boolean =
    s == disp
}
...
class AfterTimeTrigger(val time: Double) : MarkerTrigger {
  override fun shouldTrigger(robotState: RobotState, trajectory: Trajectory<*>, s: Double): Boolean =
    s > trajectory.wrtTime().profile[time].value()
}

@Serializable
@SerialName("AtPoint")
class AtPointTrigger @JvmOverloads constructor(val point: Vector2d, val tolerance: Double = 2.0) :
  MarkerTrigger {
  override fun shouldTrigger(robotState: RobotState, trajectory: Trajectory<*>, s: Double): Boolean =
    (robotState.pose.position - point).norm() < tolerance
}

@Serializable
@SerialName("AtPose")
class AtPoseTrigger @JvmOverloads constructor(
  val pose: Pose2d,
  val linearTolerance: Double = 2.0,
  val angularTolerance: Double = Math.toRadians(5.0),
) : MarkerTrigger {
  override fun shouldTrigger(robotState: RobotState, trajectory: Trajectory<*>, s: Double): Boolean = (
    robotState.pose -
      pose
    ).let { it.line.norm() < linearTolerance && it.angle < angularTolerance }
}
...
class Marker(
  ...
  companion object {
    ...
    @JvmStatic
    @JvmOverloads
    fun atPoint(point: Vector2d, tolerance: Double = 2.0, callback: MarkerCallback) =
      Marker(AtPointTrigger(point, tolerance), callback)
    ...
    ) = Marker(AtPoseTrigger(pose, linearTolerance, angularTolerance), callback)
  }
