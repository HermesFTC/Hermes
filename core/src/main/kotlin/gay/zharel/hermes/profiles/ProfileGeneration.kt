      ...
      when {
        currentVel >= maxVel -> {
          ...
          if (projectedEndVel <= maxVel) {
            ...
            addAccelerationSegment(
              newDisps,
              vels,
              accels,
              beginDisp + accelDistance,
              maxVel,
              maxAccel,
            )
            addConstantVelocitySegment(newDisps, vels, accels, endDisp, maxVel)
...
private fun calculateVelocityAfterAcceleration(
  ...
  acceleration: Double,
  distance: Double,
): Double = sqrt(initialVel * initialVel + 2 * acceleration * distance)

private fun calculateDistanceToReachVelocity(
  initialVel: Double,
  targetVel: Double,
  acceleration: Double,
): Double = (targetVel * targetVel - initialVel * initialVel) / (2 * acceleration)
