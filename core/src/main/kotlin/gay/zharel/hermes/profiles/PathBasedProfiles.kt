...
fun createOptimalDisplacementProfile(
  ...
  val forwardProfile =
    generatePathBasedForwardProfile(params, path, beginVel, velConstraint, accelConstraint)
  val backwardProfile =
    generatePathBasedBackwardProfile(params, path, beginVel, velConstraint, accelConstraint)
...
fun generatePathBasedBackwardProfile(
  ...
  val samples = generatePathSamples(path, params)
  val (maxVels, minAccels) = evaluateBackwardConstraintsAtSamples(
    path,
    samples,
    velConstraint,
    accelConstraint,
...
fun createSimpleDisplacementProfile(
  ...
  val forwardProfile =
    generateForwardProfile(range(0.0, length, samples + 1), beginVel, maxVels, maxAccels)
  val backwardProfile =
    generateSimpleBackwardProfile(length, maxVel, beginVel, { s -> -maxAccel(s) }, resolution)
