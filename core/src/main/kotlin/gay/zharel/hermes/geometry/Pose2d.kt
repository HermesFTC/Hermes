...
data class Pose2d(
  ...
  constructor(
    ...
    positionY: Double,
    heading: Double,
  ) : this(Vector2d(positionX, positionY), heading)
...
data class Pose2dDual<Param : DualParameter>(
  ...
  operator fun times(p: Pose2dDual<Param>) = Pose2dDual(
    heading * p.position + position,
    heading * p.heading,
  )
...
fun <Param : DualParameter> lerpPose2dDual(
  start: Pose2dDual<Param>,
  end: Pose2dDual<Param>,
  t: Double,
): Pose2dDual<Param> {
  require(t in 0.0..1.0) { "Interpolation parameter t must be between 0.0 and 1.0, but was $t" }
