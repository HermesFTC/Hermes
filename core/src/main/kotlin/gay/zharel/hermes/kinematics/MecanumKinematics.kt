...
data class MecanumKinematics @JvmOverloads constructor(
  ...
  override fun <Param : DualParameter> forward(increments: MecanumWheelIncrements<*>): Twist2dDual<Param> {
    ...
    return Twist2dDual(
      Vector2dDual(
        (increments.leftFront + increments.leftBack + increments.rightBack + increments.rightFront) *
          0.25,
        (-increments.leftFront + increments.leftBack - increments.rightBack + increments.rightFront) *
          (0.25 / lateralMultiplier),
      ),
      (-increments.leftFront - increments.leftBack + increments.rightBack + increments.rightFront) *
        (0.25 / effectiveRadius),
    )
  }
  ...
  override fun <Param : DualParameter> forward(velocities: WheelVelocities<*>): PoseVelocity2dDual<Param> {
    ...
    return PoseVelocity2dDual(
      Vector2dDual(
        (velocities.leftFront + velocities.leftBack + velocities.rightBack + velocities.rightFront) *
          0.25,
        (-velocities.leftFront + velocities.leftBack - velocities.rightBack + velocities.rightFront) *
          (0.25 / lateralMultiplier),
      ),
  ...
  override fun <Param : DualParameter> inverse(velocity: PoseVelocity2dDual<Param>) =
    MecanumWheelVelocities(
