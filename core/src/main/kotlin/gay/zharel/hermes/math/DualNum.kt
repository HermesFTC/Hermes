...
class DualNum<Param : DualParameter>(private val values: DoubleArray) {
  ...
  companion object {
    ...
    @JvmStatic
    fun <Param : DualParameter> cons(x: Double, d: DualNum<Param>) = DualNum<Param>(
  ...
  operator fun times(c: Vector2d) = Vector2dDual(this * c.x, this * c.y)
}

fun <Param : DualParameter> lerpDual(
  x: Double,
  fromLo: Double,
  fromHi: Double,
  toLo: DualNum<Param>,
...
fun <Param : DualParameter> atan2(y: DualNum<Param>, x: DualNum<Param>): DualNum<Param> {
  ...
  result[3] =
    (denom * denom * numerator - (denom * dfdt - f * dgdt) * denomDerivative) / (denom * denom * denom)
