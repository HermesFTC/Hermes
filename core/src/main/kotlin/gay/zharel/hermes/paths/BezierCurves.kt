...
data class BezierCurve1d(val coefficients: List<Double>) {
  ...
  operator fun get(t: Double): Double = (0..dim).sumOf { i ->
    cache[i] * (1 - t).pow(dim - i) * t.pow(i) * coefficients[i]
  }

  operator fun get(t: Double, n: Int): DualNum<Internal> = DualNum<Internal>(
    DoubleArray(n) {
      nthDerivative(it)[t]
    },
  )

  fun derivative(): BezierCurve1d = BezierCurve1d(
    List(dim) { i ->
  ...
  companion object {
    ...
    fun fromRRSpline(spline: QuinticSpline1d): BezierCurve1d = BezierCurve1d(
      doubleArrayOf(
        spline.f,
