...
fun poseEqual(other: Pose2d) = Matcher<Pose2d> { value ->
  MatcherResult(
    ...
    {
      "Pose2d should have had position (${other.position.x}, ${other.position.y}) " +
        "and heading (${other.heading.real}, ${other.heading.imag}), " +
        "but had position (${value.position.x}, ${value.position.y}) " +
        "and heading (${value.heading.real}, ${value.heading.imag})"
    },
    {
      "Pose2d should not have had position (${other.position.x}, ${other.position.y}) " +
        "and heading (${other.heading.real}, ${other.heading.imag})"
    },
  )
}
...
fun <P : DualParameter> dualVectorEqual(other: Vector2dDual<P>) = Matcher<Vector2dDual<P>> { value ->
  MatcherResult(
    ...
    {
      "Vector2dDual should have had x ${other.x.values()} and y ${other.y.values()}, " +
        "but had x ${value.x.values()} and y ${value.y.values()}"
    },
...
fun <P : DualParameter> dualRotationEqual(other: Rotation2dDual<P>) = Matcher<Rotation2dDual<P>> { value ->
  MatcherResult(
    ...
    {
      "Rotation2dDual should have had real ${other.real.values()} and imag ${other.imag.values()}, " +
        "but had real ${value.real.values()} and imag ${value.imag.values()}"
    },
...
fun <P : DualParameter> dualPoseEqual(other: Pose2dDual<P>) = Matcher<Pose2dDual<P>> { value ->
  MatcherResult(
    ...
    {
      "Pose2dDual should have had position.x ${other.position.x.values()}, " +
        "position.y ${other.position.y.values()}, " +
    ...
    {
      "Pose2dDual should not have had position.x ${other.position.x.values()}, " +
        "position.y ${other.position.y.values()}, " +
        "heading.real ${other.heading.real.values()}, and heading.imag ${other.heading.imag.values()}"
    },
  )
